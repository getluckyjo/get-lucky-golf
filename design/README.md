...
in beta. It has never been designed — the look grew out of the build. We want you to reskin
it for the public beta, and you may restructure the layout of any screen. The routes and
the order of the journey stay as they are.
...
Both already contain today's values. **Blank means unchanged.** Read
`01-tokens/README.md` first — particularly the contrast table, which explains why gold
cannot be used as text on cream.
...
`03-assets/ASSETS.md` has the exact list and export settings. These two are the highest-value
items in the pack: the current logo is a machine trace in the wrong green, and thirteen
emoji are currently doing the work of an icon set.
